...
void EncryptDataToFile(const std::string& output_path, const std::string& data,
  ...
  crypto_secretstream_xchacha20poly1305_init_push(&st, header, key.key.data());
  ...
  while (input_stream) {
    ...
    crypto_secretstream_xchacha20poly1305_push(
        &st, buf_out, &out_len, buf_in, input_stream.gcount(), nullptr, 0, tag);
...
std::string DecryptFromFile(const std::string input_path,
  ...
  if (!input_file.read((char*)salted_pwd.salt.data(), salted_pwd.salt.size())) {
  ...
  while (input_file) {
    ...
    if (crypto_secretstream_xchacha20poly1305_pull(&st, buf_out, &out_len, &tag,
                                                   buf_in, input_file.gcount(),
                                                   nullptr, 0) != 0) {
      throw std::runtime_error{"corrupted chunk"};
    }
    if (tag == crypto_secretstream_xchacha20poly1305_TAG_FINAL && input_file) {
