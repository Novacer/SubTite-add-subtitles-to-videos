...
Ruler::Ruler(QWidget* parent, std::chrono::milliseconds duration,
  ...
  add_subtitle_before_ = new QAction(tr("Add Subtitle Before Indicator"), this);
...
void Ruler::updateChildren() {
  ...
  for (auto& interval : subtitle_intervals_->intervals()) {
    ...
    interval->MoveBeginMarker(begin_time, millisecondsToPosition(begin_time));
    auto end_time = interval->GetEndTime();
    interval->MoveEndMarker(end_time, millisecondsToPosition(end_time));
  }
  indicator_->move(indicator_time_.count() * interval_width_ / msPerInterval(),
                   indicator_->y());
    ...
    if (event->type() == QEvent::MouseButtonPress) {
      ...
      if (watched == indicator_ && !playing_) {
        if (indicator_->x() + dx <= rect_width_ - CUT_MARKER_WIDTH &&
            indicator_->x() + dx >= 0) {
          qreal new_indicator_time = (indicator_->x() + dx) / lengthPerMs();
          indicator_time_ =
      ...
      if (auto end_marker = interval->GetEndMarker(); watched == end_marker) {
        if (end_marker->x() + dx <= rect_width_ + CUT_MARKER_WIDTH &&
          ...
          interval->MoveEndMarker(new_end_marker_time,
      ...
      if (watched == indicator_ && !playing_) {
        ...
        emit userChangedIndicatorTime(indicator_time_);
                     ...
                     subtitle_intervals_->GetIntervalFromMarker(watched);
                 interval != Q_NULLPTR) {
        emit changeSubtitleIntervalTimeFinished(subtitle_intervals_, interval);
      }
    }
  } else if (auto* interval = subtitle_intervals_->GetIntervalFromRect(watched);
             interval != Q_NULLPTR) {
