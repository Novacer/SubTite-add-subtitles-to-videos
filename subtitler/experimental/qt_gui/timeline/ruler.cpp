...
void Ruler::updateChildren() {
    ...
    mBeginMarker->move(
        mBeginMarkerTime * mIntervalLength / secondsPerInterval(),
        mBeginMarker->y());
    mEndMarker->move(mEndMarkerTime * mIntervalLength / secondsPerInterval(),
                     mEndMarker->y());
    mRectBox->setGeometry(
        mBeginMarker->x() + CUT_MARKER_WIDTH, mBeginMarker->y(),
        ...
        if (event->type() == QEvent::MouseButtonPress) {
            ...
            if (watched == mBeginMarker) {
                if (mBeginMarker->x() + dx + CUT_MARKER_WIDTH <= mRectWidth &&
                    mBeginMarker->x() + dx >= 0 &&
                    mBeginMarker->x() + dx + CUT_MARKER_WIDTH <=
                        mEndMarker->x()) {
                    mBeginMarkerTime =
                        (mBeginMarker->x() + dx) / lengthPerSecond();
                    mBeginMarker->move(mBeginMarker->x() + dx,
                                       mBeginMarker->y());
                    updateRectBox();
                }
            }
            if (watched == mEndMarker) {
                if (mEndMarker->x() + dx <= mRectWidth + CUT_MARKER_WIDTH &&
                    mEndMarker->x() + dx >= 0 &&
                    mEndMarker->x() + dx - CUT_MARKER_WIDTH >=
                        mBeginMarker->x()) {
...
void Ruler::drawScaleRuler(QPainter* painter, QRectF rulerRect) {
    ...
    for (qreal current = mOrigin; current <= rulerEndMark;
         current += 2 * mIntervalLength) {
        qreal x1 = current;
        ...
        if (x1 + mIntervalLength <= rulerEndMark) {
            painter->drawLine(
                QLineF(x1 + mIntervalLength, y1, x2 + mIntervalLength, y2));
        }
        ...
        if (x1 + mIntervalLength - TIME_LABEL_OFFSET <= rulerEndMark) {
            painter->drawText(x1 + mIntervalLength - TIME_LABEL_OFFSET,
                              y1 - HEADER_HEIGHT / 4,
