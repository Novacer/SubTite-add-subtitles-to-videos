 ...
 * Consequently, for I/O to support Unicode in Windows we must use wcin & wcout. All uses of std::string
 * then needs to be replaced with std::wstring. Doing that means keeping separate code bases for Windows and Unix.
 * 
 * Instead, InputGetter allows input to be received as UTF-8 always, even if it is originally UTF-16.
 * So you can keep on using UTF-8 std::string on either platform.
 * Use NarrowInputGetter on Unix, WideInputGetter on Windows. See below for details.
 * 
 * If you choose to pass std::cin or std::wcin to an InputGetter, THEN DO NOT USE [w]cin >> something
 * and instead only use input_getter->getline(something).
 */
class InputGetter {
public:
 ...
 * // The input is converted to UTF-8 std::string without you needing to worry about it.
 * if (input_getter->getline(input)) { std::cout << input << std::endl; }
 */
