...
std::vector<nlohmann::json> MicrosoftCognitiveService::getTranscriptionJson(
  ...
  recognizer->Recognized.Connect([&](const SpeechRecognitionEventArgs& event) {
    if (event.Result->Reason == ResultReason::RecognizedSpeech) {
      ...
      auto json = nlohmann::json::parse(event.Result->Properties.GetProperty(
      ...
      progress_msg_callback("NOMATCH: Speech could not be recognized");
    }
  });
  ...
  recognizer->Canceled.Connect(
      [&](const SpeechRecognitionCanceledEventArgs& event) {
        if (event.Reason == CancellationReason::Error) {
          progress_msg_callback("CANCELED: ErrorDetails=" + event.ErrorDetails);
...
std::vector<TranscriptionResult> MicrosoftCognitiveService::parseJson(
  ...
  for (const auto& json : jsons) {
    ...
    for (const auto& timing : word_timings) {
      ...
      const auto duration_ms = ticksToMs(timing.at("Duration").get<int64_t>());
      result.timings.push_back(WordTimings{timing.at("Word").get<std::string>(),
