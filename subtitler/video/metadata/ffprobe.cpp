        ...
        if (stream["codec_type"] == "audio") {
            if (metadata->audio) {
                throw std::runtime_error(
                    "Warning: there is more than one audio stream");
            }
            ...
            audio_stream_info->codec_name =
                stream.at("codec_name").get<std::string>();
            audio_stream_info->sample_rate =
                std::stoi(stream.at("sample_rate").get<std::string>());
            audio_stream_info->channels = stream.at("channels").get<int>();
            audio_stream_info->start_time = StringSecondsToMilliseconds(
                stream.at("start_time").get<std::string>());
            audio_stream_info->duration = StringSecondsToMilliseconds(
                stream.at("duration").get<std::string>());
            ...
            video_stream_info->codec_name =
                stream.at("codec_name").get<std::string>();
            video_stream_info->codec_tag_string =
                stream.at("codec_tag_string").get<std::string>();
            video_stream_info->width = stream.at("width").get<int>();
            video_stream_info->height = stream.at("height").get<int>();
            video_stream_info->has_b_frames =
                stream.at("has_b_frames").get<int>();
            video_stream_info->start_time = StringSecondsToMilliseconds(
                stream.at("start_time").get<std::string>());
            video_stream_info->duration = StringSecondsToMilliseconds(
                stream.at("duration").get<std::string>());
        }
    }
...
FFProbe::FFProbe(const std::string &ffprobe_path,
    ...
    if (!executor_) {
        throw std::invalid_argument(
            "Executor provided to ffprobe cannot be null!");
    }
}
...
std::vector<std::string> FFProbe::BuildArgs() {
    std::vector<std::string> args = {"-hide_banner", "-show_format",
                                     "-loglevel error", "-show_streams",
